...
import model as m #Modify the Path based on the model being used
...
train_dl = torch.utils.data.DataLoader(train_myds, batch_size=16, shuffle=True,num_workers=4)
val_dl = torch.utils.data.DataLoader(val_ds, batch_size=16, shuffle=False, num_workers=4)

# Create the model and put it on the GPU if available

###Modify this function based on the model you are running
myModel = m.proj_model()
...
def training(model, train_dl, val_dl, num_epochs):
  ...
  for epoch in range(num_epochs):
    ...
    if best_acc < acc:
      best_acc = acc
      torch.save(model.state_dict(), 'audioModel.pt')
